package me.tulio.yang.tablist.impl.utils.impl;
...
import me.tulio.yang.tablist.impl.utils.*;
import me.tulio.yang.utilities.PlayerUtil;
import me.tulio.yang.utilities.chat.JSONMessage;
...
public class ProtocolLibTabImpl implements IRubenHelper {
    ...
    public void recreatePlayer(GhostlyTablist tablist, Player player) {
        ...
        packet.getPlayerInfoDataLists().write(0, Lists.newArrayList(playerInfoData));
    ...
    public void updateFakeSkin(GhostlyTablist zigguratTablist, TabEntry tabEntry, SkinTexture skinTexture) {
        ...
        remove.getPlayerInfoDataLists().write(0, Lists.newArrayList(playerInfoData));
        ...
        add.getPlayerInfoDataLists().write(0, Lists.newArrayList(playerInfoData));
    ...
    @Override
    public void updateHeaderAndFooter(GhostlyTablist zigguratTablist, List<String> header, List<String> footer) {
        final Player player = zigguratTablist.getPlayer();
        PacketContainer headerAndFooter = new PacketContainer(PacketType.Play.Server.PLAYER_LIST_HEADER_FOOTER);
        
        JSONMessage headerText = JSONMessage.create();
        for (String s : header) {
            headerText.then(s);
            if (!s.equals(header.get(header.size() - 1))) headerText.newline();
        }
        JSONMessage footerText = JSONMessage.create();
        for (String s : footer) {
            footerText.then(s);
            if (!s.equals(footer.get(footer.size() - 1))) footerText.newline();
        }

        headerAndFooter.getChatComponents().write(0, WrappedChatComponent.fromJson(headerText.toString()));
        headerAndFooter.getChatComponents().write(1, WrappedChatComponent.fromJson(footerText.toString()));
