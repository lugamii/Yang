...
public class Conversation {
	...
	public void sendMessage(Player sender, Player target, String message) {
		...
		new MessageFormat(Locale.CONVERSATION_SEND_MESSAGE.format(senderProfile.getLocale()))
			...
			.add("{sender_prefix}", Yang.get().getRankManager().getRank().getPrefix(sender.getUniqueId()))
			.add("{sender_suffix}", Yang.get().getRankManager().getRank().getSuffix(sender.getUniqueId()))
			.add("{target_name}", target.getName())
			.add("{target_displayname}", target.getDisplayName())
			.add("{target_color}", targetProfile.getColor())
			.add("{target_prefix}", Yang.get().getRankManager().getRank().getPrefix(target.getUniqueId()))
			.add("{target_suffix}", Yang.get().getRankManager().getRank().getSuffix(target.getUniqueId()))
			.add("{msg}", message)
			.send(sender);
