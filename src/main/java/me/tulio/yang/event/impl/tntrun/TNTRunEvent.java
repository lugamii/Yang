...
public class TNTRunEvent implements Event {
    ...
    public String getDisplayName() {
        return CC.translate(Yang.get().getEventsConfig().getString("EVENTS.TNTRUN.DISPLAYNAME"));
    }

    @Override
    public List<String> getDescription() {
        return Yang.get().getEventsConfig().getStringList("EVENTS.TNTRUN.DESCRIPTION");
    }
    ...
    public ItemStack getIcon() {
        return new ItemBuilder(Material.valueOf(Yang.get().getEventsConfig().getString("EVENTS.TNTRUN.ICON"))).build();
    }

    @Override
    public boolean canHost(Player player) {
        return player.hasPermission("yang.event.host.tntrun");
    }
